-- AI or Nah - Monetization Schema (Fixed)
-- This migration:
-- 1. Makes bottom_line nullable (it's deprecated but still in old schema)
-- 2. Creates monetization tables

-- Fix existing results table
ALTER TABLE IF EXISTS results
  ALTER COLUMN bottom_line DROP NOT NULL;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_updated_at') THEN
    ...
    DROP TRIGGER IF EXISTS device_fingerprints_updated_at ON device_fingerprints;
