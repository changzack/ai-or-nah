...
/**
 * Mock AI Detection integration for development without API keys
...
export async function mockAnalyzeImage(
  ...
  // Simulate API delay (Sightengine is usually fast ~500ms per image)
  await new Promise((resolve) => setTimeout(resolve, 300 + Math.random() * 400));

  const aiProbability = generateMockScore(imageUrl);

  console.log(
    `[AI Detection Mock] Image analyzed: ${imageUrl.substring(0, 50)}... - AI probability: ${(aiProbability * 100).toFixed(1)}%`
  );
...
export async function mockAnalyzeImages(
  ...
  console.log(`[AI Detection Mock] Analyzing ${imageUrls.length} images...`);

  // Analyze in parallel like the real implementation
  const promises = imageUrls.map((url) => mockAnalyzeImage(url));
  const results = await Promise.all(promises);

  console.log(`[AI Detection Mock] Completed: ${results.length}/${imageUrls.length} images`);
...
export function getMockInstructions(): string {
  return `
Mock AI Detection integration active (no API keys configured).
...
To use real Sightengine API, set SIGHTENGINE_API_USER and SIGHTENGINE_API_SECRET in .env.local
  `.trim();
}
